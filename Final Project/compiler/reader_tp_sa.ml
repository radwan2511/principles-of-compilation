...
module Reader : READER = struct
  ...
  and nt_frac str =
    ...
    let nt1 = pack nt1
                (fun (num, den) ->
                  (* fixed: pass absulote value to gcd*)
...
let rec let_params ribs = 
  match ribs with 
    | ScmNil -> (ScmNil,ScmNil)
    | ScmPair(ScmPair(param,ScmPair(value,ScmNil)),cdr) ->
      let (a,v) = (let_params cdr) in
      (ScmPair(param,a),ScmPair(value,v))
   | _ -> raise (X_syntax "Error");;

let let_help (ribs,exprs) =    
  let (ar,vals) = (let_params ribs) in
  ScmPair(ScmPair(ScmSymbol "lambda",ScmPair (ar,exprs)),vals);;
...
let letrec_help ( f , s ) =
   ...
   match args with 
   | ScmNil -> (ScmNil,s)
   | ScmPair(ScmPair(ScmSymbol f,ScmPair(s,ScmNil)),cdr) -> (let (x,y) = pars cdr in 
   (ScmPair(ScmPair(ScmSymbol f,ScmPair(ScmPair(ScmSymbol "quote",ScmPair(ScmSymbol "whatever",ScmNil)),ScmNil)),x),ScmPair(ScmPair(ScmSymbol "set!",ScmPair(ScmSymbol f,ScmPair(s,ScmNil))),y)))
    | _ -> raise (X_syntax "Error") in
   let (p,v) = pars f in 
   ScmPair (ScmSymbol "let",ScmPair(p,v));;


let rec or_help = function
| ScmPair (car, cdr) -> car::(or_help cdr)
| ScmNil -> []
| sexpr ->  [];;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
         ...
         tag_parse (let_help (ribs,exprs))
                  

      (* added or tag parse*)

      | (ScmPair (ScmSymbol "or", ScmNil)) -> tag_parse (ScmBoolean false)
      | (ScmPair (ScmSymbol "or", ScmPair (x, ScmNil))) -> (tag_parse x)
      | (ScmPair (ScmSymbol "or", exprs)) -> ScmOr(List.map tag_parse (or_help exprs))
    ...
    | ScmPair (ScmSymbol "let*",
      ...
      | ScmPair (ScmSymbol "let*",
               ScmPair (ScmPair (ScmPair (var, ScmPair (arg, ScmNil)),
                        ...
                        tag_parse ( ScmPair (ScmSymbol "let", ScmPair (ScmPair (ScmPair (var, ScmPair (arg, ScmNil)),  ScmNil),  ScmPair (ScmPair
                     (ScmSymbol "let*", ScmPair (ribs, exprs)), ScmNil))) )
                     

    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       
       tag_parse(letrec_help (ribs, exprs))
...
(***)
let rec var_helper_get name = function 
   | [] -> raise(X_this_should_not_happen "env doesn't contain var!")
   |rib :: env -> 
      if (not ((List.mem name rib)))
         then var_helper_get name env
         else (rib);;

let env_helper_equals name sviva1 sviva2 = 
      (var_helper_get name sviva1) == (var_helper_get name sviva2);;
(***)
...
let should_box_var name expr params = 
   ...
   let x_reads_x_writes_x = cross_product reads writes in
   let rec run = function 
   | [] -> false

   | ((Var' (s1, Param(k1) ),sviva1) ,
       (Var' (s2, Param(k2) ),sviva2)) :: rest -> run rest
   
   | ((Var' (s1, Param(k1) ),sviva1) ,
       (Var' (s2, Bound(m2,k2) ),sviva2)) :: rest -> true

   | ((Var' (s1, Bound(m1,k1)),sviva1) ,
       (Var' (s2, Param(k2)),sviva2)) :: rest -> true

   |((Var' (s1, Bound(m1,k1)),sviva1) ,
       (Var' (s2, Bound(m2,k2)),sviva2)) :: rest -> not(not( not ((env_helper_equals name sviva1 sviva2)  ) ))  || (run rest) 
   | _ :: rest -> run rest
   
   in run x_reads_x_writes_x;;
...
let box_sets_and_gets name body =
  ...
  let rec auto_box expr =
    ...
    | ScmLambda' (params, Opt opt, expr') ->
      ...
      let box_params = params @ [opt] in
      let box_these =
         List.filter
           (fun param -> should_box_var param expr' box_params)
           box_params in
       let new_body = 
         List.fold_left
           (fun body name -> box_sets_and_gets name body)
           (auto_box expr')
           box_these in
       let new_sets = make_sets box_these box_params in
       let new_body = 
         match box_these, new_body with
         | [], _ -> new_body
         | _, ScmSeq' exprs -> ScmSeq' (new_sets @ exprs)
